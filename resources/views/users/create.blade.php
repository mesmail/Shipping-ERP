@extends('layouts.app')
@section('title', app()->getLocale() === 'ar' ? 'مستخدم جديد' : 'New User')
@section('page-title', app()->getLocale() === 'ar' ? 'مستخدم جديد' : 'New User')
@section('content')
<div class="max-w-xl space-y-4">
    <div class="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span class="text-gray-900">{{ app()->getLocale() === 'ar' ? 'جديد' : 'New' }}</span>
    </div>
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('users.store') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="form-label">{{ app()->getLocale() === 'ar' ? 'كلمة المرور' : 'Password' }} <span class="text-red-500">*</span></label>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select name="branch_id" class="form-select">
                            ...
                            @foreach($branches as $branch)
                                <option value="{{ $branch->id }}" {{ old('branch_id') == $branch->id ? 'selected' : '' }}>{{ $branch->display_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="sm:col-span-2">
