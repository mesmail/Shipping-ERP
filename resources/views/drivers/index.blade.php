@extends('layouts.app')

@section('title', app()->getLocale() === 'ar' ? 'السائقون' : 'Drivers')
@section('page-title', app()->getLocale() === 'ar' ? 'السائقون' : 'Drivers')

@section('content')
<div class="space-y-4">

    {{-- Header --}}
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
            {{-- Breadcrumb --}}
            <nav class="flex items-center gap-2 text-sm text-gray-500 mb-1">
                <a href="{{ route('dashboard') }}" class="hover:text-blue-600">
                    <i class="fa-solid fa-house text-xs"></i>
                </a>
                <i class="fa-solid fa-chevron-{{ app()->getLocale() === 'ar' ? 'left' : 'right' }} text-xs"></i>
                <span class="text-gray-900 font-medium">{{ app()->getLocale() === 'ar' ? 'السائقون' : 'Drivers' }}</span>
            </nav>
            <h2 class="text-xl font-bold text-gray-900">{{ app()->getLocale() === 'ar' ? 'قائمة السائقين' : 'Driver List' }}</h2>
            <p class="text-sm text-gray-500 mt-0.5">{{ $drivers->total() }} {{ app()->getLocale() === 'ar' ? 'نتيجة' : 'results' }}</p>
        </div>
        <a href="{{ route('drivers.create') }}" class="btn-primary">
            <i class="fa-solid fa-plus"></i>
            {{ app()->getLocale() === 'ar' ? 'إضافة سائق' : 'Add Driver' }}
        </a>
    </div>

    {{-- Flash Messages --}}
    @if(session('success'))
        <div class="bg-green-50 border border-green-200 text-green-800 rounded-xl px-4 py-3 flex items-center gap-3">
            <i class="fa-solid fa-circle-check text-green-500"></i>
            <span class="text-sm font-medium">{{ session('success') }}</span>
        </div>
    @endif
    @if(session('error'))
        <div class="bg-red-50 border border-red-200 text-red-800 rounded-xl px-4 py-3 flex items-center gap-3">
            <i class="fa-solid fa-circle-xmark text-red-500"></i>
            <span class="text-sm font-medium">{{ session('error') }}</span>
        </div>
    @endif

    {{-- Search --}}
    <div class="card">
        <div class="p-4">
            <form method="GET" action="{{ route('drivers.index') }}">
                <div class="flex flex-col sm:flex-row gap-3">
                    <div class="flex-1 relative">
                        <i class="fa-solid fa-search absolute {{ app()->getLocale() === 'ar' ? 'right-3' : 'left-3' }} top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
                        <input type="text" name="search" value="{{ request('search') }}"
                               placeholder="{{ app()->getLocale() === 'ar' ? 'ابحث بالاسم أو الهاتف...' : 'Search by name or phone...' }}"
                               class="form-input {{ app()->getLocale() === 'ar' ? 'pr-10' : 'pl-10' }}">
                    </div>
                    <div class="flex gap-2">
                        <button type="submit" class="btn-primary flex-shrink-0">
                            <i class="fa-solid fa-search"></i>
                            {{ app()->getLocale() === 'ar' ? 'بحث' : 'Search' }}
                        </button>
                        @if(request('search'))
                            <a href="{{ route('drivers.index') }}" class="btn-secondary flex-shrink-0">
                                <i class="fa-solid fa-xmark"></i>
                                {{ app()->getLocale() === 'ar' ? 'مسح' : 'Clear' }}
                            </a>
                        @endif
                    </div>
                </div>
            </form>
        </div>
    </div>

    {{-- Table --}}
    <div class="card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="data-table w-full">
                <thead>
                    <tr>
                        <th class="{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">#</th>
                        <th class="{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
                            {{ app()->getLocale() === 'ar' ? 'الاسم' : 'Name' }}
                        </th>
                        <th class="{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
                            {{ app()->getLocale() === 'ar' ? 'الهاتف' : 'Phone' }}
                        </th>
                        <th class="{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
                            {{ app()->getLocale() === 'ar' ? 'الفرع' : 'Branch' }}
                        </th>
                        <th class="{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
                            {{ app()->getLocale() === 'ar' ? 'رقم المركبة' : 'Vehicle No.' }}
                        </th>
                        <th class="{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
                            {{ app()->getLocale() === 'ar' ? 'الحالة' : 'Status' }}
                        </th>
                        <th class="{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
                            {{ app()->getLocale() === 'ar' ? 'الشحنات' : 'Shipments' }}
                        </th>
                        <th class="{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
                            {{ app()->getLocale() === 'ar' ? 'الإجراءات' : 'Actions' }}
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($drivers as $driver)
                        <tr>
                            <td class="text-gray-400 text-xs">{{ $driver->id }}</td>
                            <td>
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-semibold text-sm flex-shrink-0">
                                        {{ mb_substr($driver->name, 0, 1) }}
                                    </div>
                                    <div>
                                        <div class="font-medium text-gray-900">{{ $driver->name }}</div>
                                        @if($driver->id_number)
                                            <div class="text-xs text-gray-400">{{ $driver->id_number }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div class="text-sm text-gray-900">{{ $driver->phone }}</div>
                                @if($driver->phone_alt)
                                    <div class="text-xs text-gray-400">{{ $driver->phone_alt }}</div>
                                @endif
                            </td>
                            <td class="text-sm text-gray-600">
                                @if($driver->branch)
                                    <span class="inline-flex items-center gap-1">
                                        <i class="fa-solid fa-building text-xs text-gray-400"></i>
                                        {{ $driver->branch->display_name }}
                                    </span>
                                @else
                                    <span class="text-gray-400">—</span>
                                @endif
                            </td>
                            <td>
                                @if($driver->vehicle_number)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded font-mono text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-200">
                                        {{ $driver->vehicle_number }}
                                    </span>
                                @else
                                    <span class="text-gray-400">—</span>
                                @endif
                            </td>
                            <td>
                                @if($driver->is_active)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">
                                        <span class="w-1.5 h-1.5 rounded-full bg-green-500 {{ app()->getLocale() === 'ar' ? 'ml-1.5' : 'mr-1.5' }}"></span>
                                        {{ app()->getLocale() === 'ar' ? 'نشط' : 'Active' }}
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">
                                        <span class="w-1.5 h-1.5 rounded-full bg-gray-400 {{ app()->getLocale() === 'ar' ? 'ml-1.5' : 'mr-1.5' }}"></span>
                                        {{ app()->getLocale() === 'ar' ? 'غير نشط' : 'Inactive' }}
                                    </span>
                                @endif
                            </td>
                            <td>
                                <span class="inline-flex items-center gap-1 text-sm font-semibold text-gray-700">
                                    <i class="fa-solid fa-boxes-stacked text-xs text-gray-400"></i>
                                    {{ $driver->shipments_count ?? 0 }}
                                </span>
                            </td>
                            <td>
                                <div class="flex items-center gap-1">
                                    <a href="{{ route('drivers.edit', $driver) }}"
                                       class="btn-icon btn-secondary" title="{{ app()->getLocale() === 'ar' ? 'تعديل' : 'Edit' }}">
                                        <i class="fa-solid fa-pen text-sm"></i>
                                    </a>
                                    <form method="POST" action="{{ route('drivers.destroy', $driver) }}"
                                          onsubmit="return confirm('{{ app()->getLocale() === 'ar' ? 'هل أنت متأكد من حذف هذا السائق؟' : 'Are you sure you want to delete this driver?' }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                class="btn-icon btn-danger" title="{{ app()->getLocale() === 'ar' ? 'حذف' : 'Delete' }}">
                                            <i class="fa-solid fa-trash text-sm"></i>
                                        </button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-16 text-gray-400">
                                <i class="fa-solid fa-car text-5xl mb-3 block"></i>
                                <p class="text-sm">{{ app()->getLocale() === 'ar' ? 'لا يوجد سائقون بعد' : 'No drivers yet' }}</p>
                                @if(request('search'))
                                    <a href="{{ route('drivers.index') }}" class="text-blue-600 hover:underline text-xs mt-1 inline-block">
                                        {{ app()->getLocale() === 'ar' ? 'عرض الكل' : 'Show all' }}
                                    </a>
                                @endif
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>

        @if($drivers->hasPages())
            <div class="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
                <p class="text-sm text-gray-500">
                    {{ app()->getLocale() === 'ar' ? 'عرض' : 'Showing' }}
                    {{ $drivers->firstItem() }}-{{ $drivers->lastItem() }}
                    {{ app()->getLocale() === 'ar' ? 'من' : 'of' }}
                    {{ $drivers->total() }}
                    {{ app()->getLocale() === 'ar' ? 'نتيجة' : 'results' }}
                </p>
                {{ $drivers->withQueryString()->links() }}
            </div>
        @endif
    </div>
</div>
@endsection
